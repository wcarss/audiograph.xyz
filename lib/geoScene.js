...
module.exports = function ({ renderer, camera, scene, palettes, envMap, loop }) {
  ...
  for (let i = 0; i < TOTAL_PARTS; i++) {
    const mesh = addCore({ active: i < INITIAL_PARTS, type: Math.random() > 0.78 ? 0 : 1});
  ...
  function addCore (opt = {}) {
    ...
    if (opt.type === 0) {
      ...
      const pieceSize = random(0.05, 0.25);
      ...
      let numSlices = Math.floor(random(15, 45));
      if (Math.random() > 0.4) {
        numSlices = Math.floor(random(2, 6));
      }
      ...
      mesh = addGeom(geoArc({
        ...
        innerRadius: radius,
        outerRadius: radius + random(0.005, 0.25),
        numBands: Math.floor(random(2, 6)),
        numSlices: numSlices,
      }), opt);
    }
